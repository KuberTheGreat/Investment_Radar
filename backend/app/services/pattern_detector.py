...
async def detect_patterns_for_symbol(
    symbol: str, timeframe: str = "15m", limit: int = 50
):
    """
    ...
    async with AsyncSessionLocal() as session:
        ...
        stmt = (
            select(OHLCCandle)
            .where(OHLCCandle.symbol == symbol, OHLCCandle.timeframe == timeframe)
            .order_by(OHLCCandle.timestamp.desc())
            .limit(limit)
        )
        ...
        current_trend_up = (
            close_data[-1] > sma_20[-1] if not np.isnan(sma_20[-1]) else True
        )
        ...
        for p in PATTERNS_CONFIG:
            ...
            for idx in non_zero_indices:
                ...
                if timeframe == "1d":
                    ...
                    idx_trend_up = (
                        close_data[idx] > sma_20[idx]
                        if not np.isnan(sma_20[idx])
                        else current_trend_up
                    )
