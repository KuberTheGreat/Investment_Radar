...
async def get_watchlist(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Watchlist.symbol).where(Watchlist.user_id == current_user.id))
    symbols = result.scalars().all()
    return symbols

@router.post("/{symbol}")
async def add_to_watchlist(symbol: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    clean_sym = symbol.upper().replace(".NS", "").replace(".BO", "")
    # Check if already exists securely avoiding duplications
    result = await db.execute(
        select(Watchlist).where(Watchlist.user_id == current_user.id, Watchlist.symbol == clean_sym)
    )
...
async def remove_from_watchlist(symbol: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    clean_sym = symbol.upper().replace(".NS", "").replace(".BO", "")
    stmt = delete(Watchlist).where(Watchlist.user_id == current_user.id, Watchlist.symbol == clean_sym)
