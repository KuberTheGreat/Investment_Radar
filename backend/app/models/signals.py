...
class Signal(Base):
    ...
    __table_args__ = (UniqueConstraint("pattern_id", name="uq_signals_pattern_id"),)

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    symbol = Column(String(20), nullable=False)
    signal_type = Column(String(20), nullable=False)
    pattern_id = Column(
        UUID(as_uuid=True), ForeignKey("detected_patterns.id"), nullable=True
    )
    event_ids = Column(
        ARRAY(UUID(as_uuid=True)), nullable=True
    )  # Array of FK references
