...
from app.models.broker import (
    BrokerSession,
)  # Ensure Alembic autogenerates the migration
...
async def lifespan(app: FastAPI):
    # Startup
    scheduler.add_job(
        run_market_data_pipeline, "interval", minutes=15, id="market_data_job"
    )
    scheduler.add_job(
        run_pattern_detector, "interval", minutes=15, id="pattern_job", max_instances=1
    )
    scheduler.add_job(
        run_corporate_events_pipeline,
        "interval",
        minutes=60,
        id="corporate_events_job",
        max_instances=1,
    )
    scheduler.add_job(
        run_confluence_scorer, "interval", minutes=15, id="scorer_job", max_instances=1
    )
    scheduler.add_job(
        run_opportunity_radar, "interval", minutes=15, id="radar_job", max_instances=1
    )
    # Signal expiry: runs every day at midnight IST (18:30 UTC)
    from app.api.endpoints import expire_old_signals

    scheduler.add_job(
        expire_old_signals, "cron", hour=18, minute=30, id="signal_expiry_job"
    )
    # Angel One daily token refresh: 06:00 IST = 00:30 UTC, before NSE open
    scheduler.add_job(
        broker_service.refresh_session,
        "cron",
        hour=0,
        minute=30,
        id="broker_refresh_job",
    )
    scheduler.start()
    ...
    async def _start_broker_ws():
        try:
            ...
            logger.info(
                f"broker_ws: Auto-started with {len(symbols)} watchlist symbols."
            )
        except Exception as e:
